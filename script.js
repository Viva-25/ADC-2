...
const heroData = {
    ...
    'Storm': {
        // Corrected quote
        quote: "I'm a goddess. I'm a mutant. I'm an X-Man.",
        image: "ST.jpg",
        audioSequence: ["C5", "F5", "A5", "C6"]
    }
};
// Function to navigate between pages instantly
function showPage(pageId) {
    const pages = ['riddlePage', 'genderPage', 'maleHeroesPage', 'femaleHeroesPage', 'heroDisplayPage', 'eventDetailsPage'];

    // Loop through all page IDs to manage their visibility
    pages.forEach(id => {
        ...
        if (page) {
            ...
            // Clean up any lingering opacity classes from the old transition system
...
function checkRiddleAnswer(answer) {
    ...
    if (answer === 'Map') {
        ...
        // The line below was causing the shake. It has been commented out.
        // riddlePage.classList.add('shake-animation');

        riddleFeedback.textContent = "WRONG! TRY AGAIN!";
        riddleFeedback.classList.remove('hidden');

        // This event listener, which removed the shake class, is no longer needed.
